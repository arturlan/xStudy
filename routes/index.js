...
var questionAnswerUrlAppend = '/answers?site=stackoverflow&filter=withbody';
...
router.post('/', function(req, res, next) {
    ...
    var response = request({
        ...
        arr.forEach(function(item) {
            arrOfQuestionsIds.push(item.question_id);
            arrOfTitles.push(item.title);
            /*This is for getting answers*/
            // gettingAnswers(item.question_id);
        })
        // console.log(arrOfQuestionsIds);



        /*getting titles*/
        /*gettingTitles(arrOfQuestionsIds, arrOfTitles);*/
        for (var i = 0; i < arrOfTitles.length; i++) {
            obj[arrOfTitles[i]] = arrOfQuestionsIds[i];
...
var gettingAnswers = function(id) {
    var urlForAnswers = url + 'questions/' + id + questionAnswerUrlAppend;
    var response = request({
        method: 'GET',
        uri: urlForAnswers,
        gzip: true
        ...
        var parsedJson = JSON.parse(body);
        console.log(parsedJson.items[0].question_id);
    })
}

var renderingHtml = function(obj) {
    var arrOfAnswers = '<h1>All possible answers:</h1>';
    var css = '#login {text-align: right; } #header {margin-top: 30px;}';
    var jq = '$(document).ready(function() {$("h3").click(function () {var $stext = $(this).next("p").stop(true, true);if ( $stext.is( ":hidden" ) ) {$stext.slideDown();} else {$stext.hide();}});});'
    ...
    Object.keys(obj).forEach(function(e) {
        arrOfAnswers += '<h3>' + e + '</h3><p style="display:none">Hello kitty</p>';
    })

    var data = `<div class="row"><div class="col-xs-1 col-md-3"></div><div class="col-xs-10 col-md-6"><p>${arrOfAnswers}</p></div><div class="col-xs-1 col-md-3"></div></div>`
    var end = '</body></html>';
